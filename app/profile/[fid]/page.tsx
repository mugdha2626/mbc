    ...
    const fetchData = async () => {
      try {
        ...
        if (portfolio?.dishes) {
          ...
          const holdingsResults = (await Promise.all(holdingsPromises)).filter(
            Boolean
        ...
        if (currentUser?.fid) {
          const wishlistRes = await fetch(
            `/api/wishlist?fid=${currentUser.fid}`
          );
          if (wishlistRes.ok) {
            const wishlistData = await wishlistRes.json();
            const wishlistDishIds = new Set<string>(
              (wishlistData.wishlist || []).map(
                (item: { dish: string }) => item.dish
              )
            );
